...
#' Script aim: NERC  Telemetry project - analysis of relative onset of activity heterogeneous variances
#' 
...
##
##
data <- readRDS(file = "./data/6_radio_onset/prep_data_analysis_FILTERED_230328.RDS")
...
model_onset_activity <- brm(model_formula_onset_activity, 
                            ...
                            #backend = 'cmdstanr',
                            file_refit = 'always',
                            file = './models/brms_model_onset_activity.RDS')

# read model back if needed (no need to fit the model every time the script is run)
model_onset_activity <- readRDS('./models/brms_model_onset_activity.rds')
...
##
## save table
gtsave(table_model_onset_activity, "./tables/TABLE_onset_activity.html")
...
saveRDS(object = fig_1c, file = './plots/Figure_1c.RDS')
...
saveRDS(object = fig_1a, file = './plots/Figure_1a.RDS')
...
fig_2a_presentation <- fig_2a +
  scale_x_continuous(limits = c(0.2, 0.6))

ggsave(filename = './plots/fig2a_presentation.png', 
       plot = fig_2a_presentation, 
       units = 'mm',
       device = 'png', 
       width = 150, 
       height = 100)
