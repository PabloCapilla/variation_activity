...
##
##
onset <- readRDS(file = "./data/01_onset_data/prep_data_analysis_FILTERED_231211.RDS")
head(onset)

end <- readRDS(file = "./data/02_end_data/prep_data_analysis_FILTERED_231211.RDS")
head(end)
...
summary(data_model$duration_active_day)
...
#                            backend = 'cmdstanr',
#                            file_refit = 'always',
#                            file = './models/brms_model_duration_activity.RDS')

# read model back if needed (no need to fit the model every time the script is run)
model_duration_activity <- readRDS('./models/brms_model_duration_activity.rds')
...
##
## save table
gtsave(table_model_duration_activity, "./tables/Table S3 raw.html")
...
saveRDS(object = fig_S4b, file = './plots/Figure_S4b.RDS')
...
saveRDS(object = fig_S4c, file = './plots/Figure_S4c.RDS')
...
# plot
fig_S4a <- ggplot(data = data_model %>% 
                   mutate(species = factor(x = species, 
                                           levels =  c("Robin", "Blackbird",
                                                       "Great tit", "Blue tit",
                                                       "Dunnock", "Chaffinch")),
...
saveRDS(object = fig_S4a, file = './plots/Figure_S4a.RDS')
